...
namespace EnrollmentSystem.Core
{
    public class CourseCurriculum
    {
        ...
        // Alias for Program.cs compatibility - calls RemoveCourse
        public bool DeleteCourse(string code)
        {
            return RemoveCourse(code);
        }
        ...
        // Alias for Program.cs compatibility - calls GetTotalUnits
        ...
        /// <summary>
        /// Sorts the curriculum by course title using Bubble Sort.
        /// Time Complexity: O(n²)
        /// </summary>
        public void SortByCourseTitle()
        {
            ...
            for (int outer = 0; outer < courseArray.Length - 1; outer++)
            {
                for (int inner = 0; inner < courseArray.Length - outer - 1; inner++)
                {
                    if (string.Compare(courseArray[inner].Title, courseArray[inner + 1].Title, StringComparison.OrdinalIgnoreCase) > 0)
                    p
                        var temp = courseArray[inner];
                        courseArray[inner] = courseArray[inner + 1];
                        courseArray[inner + 1] = temp;
