...
namespace DataStructuresLibrary.Tests
{
    public class CustomQueueTests
    {
        ...
        public void Enqueue_ShouldPreserveOrder_WhenTheBufferWrapsAndGrows()
        {
            ...
            // buffer wraps before it grows. That is where an off-by-one in the
            // modulo arithmetic would show up.
            var queue = new CustomQueue<int>();
        ...
        [Fact]
        public void Peek_ShouldReturnFrontElementWithoutRemovingIt()
        {
            var queue = new CustomQueue<int>();
            queue.Enqueue(1);
            queue.Enqueue(2);
        ...
        [Fact]
        public void Peek_ShouldFollowTheFrontAfterDequeue()
        {
            var queue = new CustomQueue<int>();
            queue.Enqueue(1);
            queue.Enqueue(2);
        ...
        [Fact]
        public void IsEmpty_ShouldReturnFalse_WhenItemsAreQueued()
        {
            var queue = new CustomQueue<int>();
            queue.Enqueue(1);
        ...
        [Fact]
        public void Search_ShouldFindElementWithoutAlteringQueueOrder()
        {
            var queue = new CustomQueue<int>();
            queue.Enqueue(10);
            queue.Enqueue(20);
        ...
        [Fact]
        public void Sort_ShouldReorderQueueElementsAscending()
        {
            var queue = new CustomQueue<int>();
            queue.Enqueue(30);
            queue.Enqueue(10);
        ...
        [Fact]
        public void Sort_ShouldStillEnqueueCorrectly_AfterRealigning()
        {
            // Sort moves the buffer back to index 0, which also has to leave
            // the rear pointer somewhere sensible for the next Enqueue.
            var queue = new CustomQueue<int>();
            queue.Enqueue(1);
            queue.Enqueue(2);
            queue.Dequeue();
